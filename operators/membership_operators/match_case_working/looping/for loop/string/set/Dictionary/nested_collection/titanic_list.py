...
passenger_class = {di.get("pclass")for di in titanic_data}
print(passenger_class)

# q3 number of female passengers:-

female_passengers = [di for di in titanic_data if di.get("sex")=="female"]
print(f"female_passengers = {len(female_passengers)}")

# q4: number of survived childs:-

survived_children = [di for di in titanic_data if di.get("survived") == 1 and di.get("age") <= 13]
print(f"survived children = {len(survived_children)}")

# q5: name whose fare > 30:-

fare_of_persons = [di for di in titanic_data if di.get("fare")>30]
print(f"fare of person ={len(fare_of_persons)}")

# q6: number survived female:-

survived_female = [di.get("name") for di in titanic_data if di.get("survived")==1 and di.get("sex")=="female"]
print(f"survived female ={len(survived_female)}")
...
print(f"survived names: {survived_peoples_names}")

# q9:survived classes:-

survived_classes = {di.get("pclass") for di in titanic_data if di.get("survived") == 1}
print(f"survived classes = {survived_classes}")
...
print(len(survived_female_rate)/len(survived_female)*100)
